...
**This script** is the usual entry for the **wear glove** task (deformable glove + Shadow Hand + AIREC): run with
``--task wearglove`` (also the default here). The same tasks are available from ``teleop_joints.py`` if you prefer a single file for imitation, block, and wearglove.
...
- **Focus the Isaac Sim viewport** (the 3D window), not the terminal or Script Editor; otherwise letter keys may not arrive while some keys (e.g. reset) can still appear to work.
- With **--task wearglove**, the **deformable glove** pushes the arms; motion can look like “no teleop” until you use large steps or compare after **Backspace** reset.
...
Run (all examples use this file; ``--task`` defaults to ``wearglove``)::

    ./isaaclab.sh -p teleop_joints_wearglove.py
    ./isaaclab.sh -p teleop_joints_wearglove.py --task wearglove
    ./isaaclab.sh -p teleop_joints_wearglove.py --task imitation
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --viewer external
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --record-demonstrations
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --record-poses
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --viewer external --record-poses --begin-from demonstrations/beautiful_grasp.npz
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --control-object --object_speed 0.5
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --replay demonstrations/demo_4.npz --video --video_length 1000
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --replay demonstrations/poses.npz --replay-pose-hold 1
    ./isaaclab.sh -p teleop_joints_wearglove.py --task block --viewer external --replay demonstrations/beautiful_grasp.npz --replay-hold-sec 10
"""
...
def _carb_key_token(name: str) -> str:
    ...
    if nu.startswith("KEY_"):
        nu = nu[4:]
    # Unify names that vary by platform / carb version
    aliases = {
...
def run_teleop_joint_loop(args_cli, simulation_app, project_root: str) -> None:
    ...
    task_key = getattr(args_cli, "task", "wearglove")
    if task_key is None:
        task_key = "wearglove"
