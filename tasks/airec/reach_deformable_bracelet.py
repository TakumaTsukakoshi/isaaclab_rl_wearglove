...
class ReachDeformableBraceletEnvCfg(AIRECEnvCfg):
    ...
    - We set ``use_glove=True`` to reuse the existing **nodal rim anchor** logic (N/S/E/W) from the glove task.
    """

    # Reuse glove-style nodal rim features (N/S/E/W) for the deformable bracelet.
    use_glove: bool = True
    ...
    #: If > 1, compute rim marker positions as the mean of K nearest nodes around the chosen N/S/E/W anchor node
    #: (in the *default* mesh). This reduces visual/metric misalignment caused by a single irregular extreme node.
    rim_anchor_patch_k: int = 8

    # NOTE: These are only used by the rigid-bracelet rim helper. We keep them to avoid accidental
    # attribute errors if someone toggles ``object_type`` for debugging.
...
class ReachDeformableBraceletEnv(AIRECEnv):
    ...
    def __init__(self, cfg: ReachDeformableBraceletEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        if cfg.object_type != "deformable":
            cfg.object_type = "deformable"
        super().__init__(cfg, render_mode, **kwargs)

        # Optional smoothing for deformable rim anchors (mean over a local patch).
        self._rim_patch_idx: dict[str, torch.Tensor] = {}

        # -----------------------------
        # Buffers / state (must exist before the first reset/step)
        # -----------------------------
    ...
    def _apply_action(self) -> None:
        ...
        # 全環境の default joint position
        default_joint_pos = self.hand.data.default_joint_pos

        # finger joint だけ取り出す
        finger_target_pos = default_joint_pos[:, self.finger_joint_ids]

        # finger joint だけに position target を送る
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None = None):
        ...
        # Update optional smoothing patch indices (mean over local patch around chosen N/S/E/W node).
        if (
            getattr(self.cfg, "rim_anchor_patch_k", 1)
            and int(self.cfg.rim_anchor_patch_k) > 1
            and self.cfg.object_type == "deformable"
        ):
            k = int(self.cfg.rim_anchor_patch_k)
            self._rim_patch_idx = {
                key: self._nearest_patch_indices(int(self.anchor_idx[key]), k=k).to(self.device)
    ...
    def _bracelet_rim_goals_env_local(self, env_ids: torch.Tensor) -> None:
        ...
        body-frame offsets from :attr:`ReachDeformableBraceletEnvCfg.bracelet_rim_offset_*`. Center matches the glove task:
        ``(north + south) / 2``.
        """
        ...
        self.goal_cent_pos[env_ids] = (self.goal_east_pos[env_ids] + self.goal_west_pos[env_ids]) / 2.0
    ...
    def _compute_intermediate_values(self, reset=False, env_ids: torch.Tensor | None = None):
        ...
        if self._use_glove:
            ...
            origins = self.scene.env_origins[env_ids]
            if self._rim_patch_idx:
                nodal = self.object.data.nodal_pos_w[env_ids]  # (B, V, 3)
                self.goal_north_pos[env_ids] = nodal[:, self._rim_patch_idx["north"], :].mean(dim=1) - origins
                self.goal_south_pos[env_ids] = nodal[:, self._rim_patch_idx["south"], :].mean(dim=1) - origins
                self.goal_east_pos[env_ids] = nodal[:, self._rim_patch_idx["east"], :].mean(dim=1) - origins
                self.goal_west_pos[env_ids] = nodal[:, self._rim_patch_idx["west"], :].mean(dim=1) - origins
            else:
                self.goal_north_pos[env_ids] = self.object.data.nodal_pos_w[env_ids, self.anchor_idx["north"], :] - origins
                self.goal_south_pos[env_ids] = self.object.data.nodal_pos_w[env_ids, self.anchor_idx["south"], :] - origins
                self.goal_east_pos[env_ids] = self.object.data.nodal_pos_w[env_ids, self.anchor_idx["east"], :] - origins
                self.goal_west_pos[env_ids] = self.object.data.nodal_pos_w[env_ids, self.anchor_idx["west"], :] - origins
            self.goal_cent_pos[env_ids] = (self.goal_east_pos[env_ids] + self.goal_west_pos[env_ids]) / 2.0
