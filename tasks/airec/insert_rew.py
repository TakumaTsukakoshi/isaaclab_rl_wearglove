...
class InsertReward:
    ...
    @torch.no_grad()
    def step(self,
             pos_ee_s: torch.Tensor,   # (B,3)  edge/mouth pose in source {S}
             quat_ee_s: torch.Tensor,  # (B,4)  (wxyz)
             pos_goal_s: torch.Tensor,   # (B,3)
             quat_goal_s: torch.Tensor,  # (B,4)
        ...
        R_SE = self._quat_wxyz_to_R(quat_ee_s)      # source <- edge
        R_ES = R_SE.transpose(1, 2)                   # edge   <- source
        R_SG = self._quat_wxyz_to_R(quat_goal_s)      # source <- grip

        # tip position & forward axis in {S}  (use CAD tip offset if provided)
        p_tip_s = pos_goal_s + torch.bmm(R_SG, self._p_tip_in_G.view(1,3).expand(B,-1).unsqueeze(-1)).squeeze(-1)
        t_axis_s = torch.bmm(R_SG, self._t_axis_in_G.view(1,3).expand(B,-1).unsqueeze(-1)).squeeze(-1)  # (B,3)

        # into edge frame {E}
        v_se   = (p_tip_s - pos_ee_s).unsqueeze(-1)           # (B,3,1)
        ...
        if center_hint_s is not None:
            ...
            to_center = center_hint_s - pos_ee_s
