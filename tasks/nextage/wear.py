...
class WearEnvCfg(AIRECEnvCfg):
    ...
    default_object_pos = [0.080, 0.00, 1.12] # 0.13 # 1.07
...
class WearEnv(AIRECEnv):
    ...
    def __init__(self, cfg: WearEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # self.right_object_goal_distance = torch.zeros((self.num_envs, 3), device=self.device)
        self.right_object_goal_distance = torch.zeros((self.num_envs, 3), device=self.device)
        self.right_object_goal_euclidean_distance = torch.zeros((self.num_envs,), device=self.device)
        self.right_object_goal_rotation = torch.zeros((self.num_envs, 4), device=self.device)
        self.right_object_goal_angular_distance = torch.zeros((self.num_envs,), device=self.device)
        # self.left_object_goal_distance = torch.zeros((self.num_envs, 3), device=self.device)
        self.left_object_goal_distance = torch.zeros((self.num_envs, 3), device=self.device)
        self.left_object_goal_euclidean_distance = torch.zeros((self.num_envs,), device=self.device)
        self.left_object_goal_rotation = torch.zeros((self.num_envs, 4), device=self.device)
        self.left_object_goal_angular_distance = torch.zeros((self.num_envs,), device=self.device)
    ...
    def _get_gt(self):
        ...
        gt = torch.cat(
            (   # xyz diffs (3,)
                ...
                # xyz diffs (3,)
                self.right_object_goal_distance,
                # rotation quaternion (4,)
                self.right_object_goal_rotation,
                # xyz diffs (3,)
                self.left_object_goal_distance,
                # rotation quaternion (4,)
                self.left_object_goal_rotation,
                # angular distances (1,)
                self.right_object_goal_angular_distance.unsqueeze(1),
                # euclidean distances (1,) [transform from (num_envs,) to (num_envs,1)]
                self.right_object_goal_euclidean_distance.unsqueeze(1),
                # angular distances (1,)
                self.left_object_goal_angular_distance.unsqueeze(1),
                # euclidean distances (1,) [transform from (num_envs,) to (num_envs,1)]
                self.left_object_goal_euclidean_distance.unsqueeze(1),
                # xyz diffs (3,)
    ...
    def _get_rewards(self) -> torch.Tensor:
        (
            ...
            self.object_pos,
            self.right_object_goal_euclidean_distance,
            self.left_object_goal_euclidean_distance,
            self.right_object_goal_angular_distance,
            self.left_object_goal_angular_distance,
            self.joint_vel,
        ...
        self.extras["log"] = {
            ...
            "r_angular_right_ee_goal": r_angular_right_object_goal,
            "r_angular_left_ee_goal": r_angular_left_object_goal,
    ...
    #     default_state[:, 7:] = torch.zeros_like(self.hand.data.default_root_state[env_ids, 7:])
    #     self.hand.write_root_state_to_sim(default_state, env_ids)
    
    # def _reset_target_pose(self, env_ids):
    #     default_state = self.hand.data.default_root_state.clone()[env_ids] # root
    ...
    #     # explicit default quaternion from hand_cfg.init_state.rot
    ...
    #     default_state[:, 3:7] = default_rot
    #     default_state[:, 7:] = torch.zeros_like(
    #         self.hand.data.default_root_state[env_ids, 7:]
    #     )
    ...
    def _reset_target_pose(self, env_ids):
        ...
        # use cfg init pos instead of whatever was in default_root_state
        init_pos = torch.tensor(
            default_state[0, 0:3],  # (0.95, 0.0, 0.95)
            device=self.device,
            dtype=default_state.dtype,
        ).unsqueeze(0).repeat(len(env_ids), 1)
    ...
    def _compute_intermediate_values(self, reset=False, env_ids: torch.Tensor | None = None):
        ...
        self.right_object_goal_distance[env_ids] = self.west_edge_pos[env_ids] - self.right_goal_pos[env_ids]
        self.right_object_goal_euclidean_distance[env_ids] = torch.norm(self.right_object_goal_distance[env_ids], dim=1)
        self.right_object_goal_rotation[env_ids] = quat_mul(self.west_edge_rot[env_ids], quat_conjugate(self.right_goal_rot[env_ids]))
        self.right_object_goal_angular_distance[env_ids] = rotation_distance(self.west_edge_rot[env_ids], self.right_goal_rot[env_ids])
        # self.left_object_goal_distance[env_ids] = self.left_l_ee_pos[env_ids] - self.left_goal_pos[env_ids]
        # print(f"east: {self.east_edge_pos[0]} left_goal_pos:{self.left_goal_pos[0]}")
        self.left_object_goal_distance[env_ids] = self.east_edge_pos[env_ids] - self.left_goal_pos[env_ids]
        self.left_object_goal_euclidean_distance[env_ids] = torch.norm(self.left_object_goal_distance[env_ids], dim=1)
        self.left_object_goal_rotation[env_ids] = quat_mul(self.east_edge_rot[env_ids], quat_conjugate(self.left_goal_rot[env_ids]))
        self.left_object_goal_angular_distance[env_ids] = rotation_distance(self.east_edge_rot[env_ids], self.left_goal_rot[env_ids])
        # print(self.left_object_goal_euclidean_distance, self.right_object_goal_euclidean_distance)
from tasks.airec.airec import distance_reward, joint_vel_penalty, stretch_reward, object_goal_reward, angular_distance_reward, insert_success_reward, success_reward
...
def compute_rewards(
    ...
    right_object_goal_euclidean_distance: torch.Tensor,
    left_object_goal_euclidean_distance: torch.Tensor,
    right_object_goal_angular_distance: torch.Tensor,
    left_object_goal_angular_distance: torch.Tensor,
    ...
    # reaching reward
    # print(f"right_object_goal_euclidean_distance {right_object_goal_euclidean_distance[0]}, left_object_goal_euclidean_distance {left_object_goal_euclidean_distance[0]}")
    r_right_object_goal = distance_reward(right_object_goal_euclidean_distance, std=0.3) * reaching_object_goal_scale
    r_left_object_goal = distance_reward(left_object_goal_euclidean_distance, std=0.3) * reaching_object_goal_scale
    r_right_ee_object = distance_reward(right_ee_object_euclidean_distance, std=0.5) * reaching_ee_object_scale  
    r_left_ee_object = distance_reward(left_ee_object_euclidean_distance, std=0.5) * reaching_ee_object_scale  
    # angular distance rewards
    r_angular_right_object_goal = angular_distance_reward(right_object_goal_angular_distance, std=0.4) * rotation_object_goal_scale
    r_angular_left_object_goal = angular_distance_reward(left_object_goal_angular_distance, std=0.4) * rotation_object_goal_scale
    r_angular_right_ee_object = angular_distance_reward(right_ee_object_angular_distance, std=0.4) * rotation_ee_object_scale 
    r_angular_left_ee_object = angular_distance_reward(left_ee_object_angular_distance, std=0.4) * rotation_ee_object_scale
    # joint velocity penalty
    r_joint_vel = joint_vel_penalty(robot_joint_vel) * joint_vel_penalty_scale
    # stretch reward
    # r_stretch = stretch_reward(ee_euclidean_distance, episode_timestep_counter, minimal_width) * stretch_object_scale
    ...
    r_object_goal = object_goal_reward(right_object_goal_euclidean_distance, r_right_insert, std=0.3) * object_goal_tracking_scale
    r_successed = success_reward(wrist_ee_distance, wrist_pos, top_pos, under_pos, minimal_distance)
    rewards = r_right_object_goal + r_left_object_goal + r_right_ee_object + r_left_ee_object + r_object_goal + r_joint_vel + r_angular_right_object_goal + r_angular_left_object_goal + r_angular_right_ee_object + r_angular_left_ee_object + r_right_insert + r_left_insert + r_successed

    return (rewards, r_right_object_goal, r_left_object_goal, r_right_ee_object, r_left_ee_object, r_object_goal, r_joint_vel, r_angular_right_ee_object, r_angular_left_ee_object, r_angular_right_ee_object, r_angular_left_ee_object, r_right_insert, r_left_insert, r_successed)
